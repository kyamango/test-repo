...
class AGCArticleGenerator:
    def __init__(self):
        ...
        # Load previously processed keywords
        self.load_processed_keywords()
    ...
    def find_niche_with_new_keywords(self):
        """
        Mencari niche pertama yang memiliki keyword baru.
        Returns: (niche_name, new_keywords) atau (None, []) jika tidak ada keyword baru
        """
        print("\n--- Checking all niches for new keywords ---")
        
        for niche in self.available_niches:
            print(f"Checking niche: {niche}")
            
            # Backup current niche state
            original_niche = self.active_niche
            original_processed = self.processed_keywords.copy()
            
            try:
                # Switch to this niche temporarily
                if self.switch_active_niche(niche):
                    # Get new keywords for this niche
                    new_keywords = self.get_new_keywords()
                    
                    if new_keywords:
                        print(f"✅ Found {len(new_keywords)} new keywords in niche: {niche}")
                        # Keep the switched niche as active since we found keywords
                        return niche, new_keywords
                    else:
                        print(f"❌ No new keywords in niche: {niche}")
                        
            except Exception as e:
                self.logger.error(f"Error checking niche {niche}: {str(e)}")
                print(f"Error checking niche {niche}: {str(e)}")
                
            # If no keywords found, restore original state for next iteration
            if niche != original_niche:
                self.active_niche = original_niche
                self.processed_keywords = original_processed
                self.update_niche_paths()
        
        print("❌ No new keywords found in any niche")
        return None, []

    def update_niche_paths(self):
        """Update all niche-related paths based on active_niche"""
        self.keywords_file = os.path.join(self.lang_path, "keywords", self.active_niche, "keywords.txt")
        self.keywords_default_file = os.path.join(self.lang_path, "keywords", self.active_niche, "keywords.default.txt")
        self.processed_file = os.path.join(self.lang_path, "keywords", self.active_niche, "processed_keywords.txt")
        self.removed_keywords_file = os.path.join(self.lang_path, "keywords", self.active_niche, "removed_keywords.txt")
    ...
    def load_keywords(self):
        """Load keywords from the active niche directory"""
        keywords_file = os.path.join(self.lang_path, "keywords", self.active_niche, "keywords.txt")
        keywords_default_file = os.path.join(self.lang_path, "keywords", self.active_niche, "keywords.default.txt")
        default_lang_path = os.path.join(self.base_path, "languages", "default")
        default_keywords_file = os.path.join(default_lang_path, "keywords", self.active_niche, "keywords.default.txt")
        
        # Create directory if it doesn't exist
        os.makedirs(os.path.dirname(keywords_file), exist_ok=True)
        
        # Try to load from regular file first
        if os.path.exists(keywords_file):
            try:
                with open(keywords_file, "r", encoding="utf-8") as f:
                    ...
                    if keywords:
                        self.logger.info(f"Loaded {len(keywords)} keywords from {keywords_file}")
                        return keywords
            except Exception as e:
                self.logger.error(f"Error reading keywords from {keywords_file}: {str(e)}")
        
        # If no keywords or file not found, try language-specific default file
        if os.path.exists(keywords_default_file):
            try:
                with open(keywords_default_file, "r", encoding="utf-8") as f:
                    keywords = f.readlines()
                    keywords = [k.strip() for k in keywords if k.strip()]
                    if keywords:
                        self.logger.info(f"Loaded {len(keywords)} keywords from default file {keywords_default_file}")
                        return keywords
            except Exception as e:
                self.logger.error(f"Error reading keywords from {keywords_default_file}: {str(e)}")
        
        # If still no keywords, try default language keywords file
        if os.path.exists(default_keywords_file):
            try:
                with open(default_keywords_file, "r", encoding="utf-8") as f:
                    keywords = f.readlines()
                    keywords = [k.strip() for k in keywords if k.strip()]
                    if keywords:
                        self.logger.info(f"Loaded {len(keywords)} keywords from default language file {default_keywords_file}")
                        return keywords
            except Exception as e:
                self.logger.error(f"Error reading keywords from {default_keywords_file}: {str(e)}")
        
        self.logger.error(f"Keywords file not found in any location for niche: {self.active_niche}")
        return []
    ...
    def load_processed_keywords(self):
        """Load the list of already processed keywords based on active niche"""
        self.processed_file = os.path.join(self.lang_path, "keywords", self.active_niche, "processed_keywords.txt")
        ...
        if os.path.exists(self.processed_file):
            with open(self.processed_file, "r", encoding="utf-8") as f:
                for line in f:
                    keyword = line.strip()
                    if keyword:
                        self.processed_keywords[keyword] = True
            self.logger.info(f"Loaded {len(self.processed_keywords)} previously processed keywords")
            print(f"Loaded {len(self.processed_keywords)} previously processed keywords")
        else:
            self.logger.info("No processed keywords file found. Will create one.")
            print("No processed keywords file found. Will create one.")
            
        # Check existing articles for duplicate keywords in the active niche content directory
        niche_content_path = os.path.join(self.hugo_content_path, self.active_niche)
        if os.path.exists(niche_content_path):
            for article_file in os.listdir(niche_content_path):
                if article_file.endswith('.md'):
                    article_path = os.path.join(niche_content_path, article_file)
                    try:
                        with open(article_path, 'r', encoding='utf-8') as f:
                            content = f.read()
                            # Extract keyword from markdown frontmatter
                            if 'keywords: [' in content:
                                keywords_text = content.split('keywords: [')[1].split(']')[0]
                                keywords = [k.strip('"\'').strip() for k in keywords_text.split(',')]
                                for kw in keywords:
                                    if kw and not kw.startswith("{") and not kw.endswith("}"):
                                        self.processed_keywords[kw] = True
                    except Exception as e:
                        self.logger.error(f"Error reading file {article_file}: {str(e)}")
                        
            self.logger.info(f"Added {len(self.processed_keywords)} keywords from existing articles")
            print(f"Added {len(self.processed_keywords)} keywords from existing articles")
    
    def save_processed_keyword(self, keyword):
        """Save a keyword as processed"""
        self.processed_keywords[keyword] = True
        with open(self.processed_file, "a", encoding="utf-8") as f:
            f.write(f"{keyword}\n")
    
    def get_new_keywords(self):
        """Get keywords that haven't been processed yet for current active niche"""
        all_keywords = self.load_keywords()
        new_keywords = [k for k in all_keywords if not self.processed_keywords.get(k, False)]
        
        if new_keywords:
            self.logger.info(f"Found {len(new_keywords)} new keywords to process in niche: {self.active_niche}")
            print(f"Found {len(new_keywords)} new keywords to process in niche: {self.active_niche}")
        else:
            self.logger.info(f"No new keywords found in niche: {self.active_niche}")
            print(f"No new keywords found in niche: {self.active_niche}")
    ...
    def switch_active_niche(self, new_niche):
        ...
        # Reload processed keywords for this niche
    ...
    def ensure_client_initialized(self):
        """Pastikan client dan sdk_type sudah di-set dengan benar"""
                ...
                for i, keyword in enumerate(new_keywords, 1):
                    ...
                    if article:
                        ...
                        if success:
                            ...
                            self.save_processed_keyword(keyword)
                            successful_count += 1
                    else:
...
def main():
    ...
    generator = None
    try:
        generator = AGCArticleGenerator()

        # Setup auto-encryption  
        setup_auto_encryption(generator)

        # Run in default mode (run-once for GitHub Actions)
        generator.run_once()
